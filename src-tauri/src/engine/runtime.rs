use serde::{Deserialize, Serialize};
use std::sync::atomic::{AtomicU64, Ordering};
use tokio::sync::Mutex;

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum AppMode {
    Mouse,
    Keyboard,
    Macro,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum RuntimePhase {
    Idle,
    Recording,
    RunningMouse,
    RunningKeyboard,
    PlayingMacro,
    Stopping,
    Error,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "kind", content = "value", rename_all = "snake_case")]
pub enum StopPolicy {
    UntilStopped,
    RepeatCount(u32),
    DurationMs(u64),
}

impl StopPolicy {
    pub fn validate(&self) -> Result<(), RuntimeError> {
        match self {
            StopPolicy::UntilStopped => Ok(()),
            StopPolicy::RepeatCount(0) => Err(RuntimeError::InvalidCount),
            StopPolicy::RepeatCount(_) => Ok(()),
            StopPolicy::DurationMs(0) => Err(RuntimeError::InvalidDuration),
            StopPolicy::DurationMs(_) => Ok(()),
        }
    }
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub enum RuntimeError {
    Busy(RuntimePhase),
    InvalidTransition,
    InvalidCount,
    InvalidDuration,
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize)]
pub struct RuntimeSnapshot {
    pub phase: RuntimePhase,
    pub selected_mode: AppMode,
    pub completed_cycles: u64,
    pub last_error: Option<String>,
}

#[derive(Debug)]
pub struct RuntimeModel {
    phase: RuntimePhase,
    selected_mode: AppMode,
    completed_cycles: u64,
    last_error: Option<String>,
}

impl Default for RuntimeModel {
    fn default() -> Self {
        Self {
            phase: RuntimePhase::Idle,
            selected_mode: AppMode::Mouse,
            completed_cycles: 0,
            last_error: None,
        }
    }
}

impl RuntimeModel {
    pub fn snapshot(&self) -> RuntimeSnapshot {
        RuntimeSnapshot {
            phase: self.phase,
            selected_mode: self.selected_mode,
            completed_cycles: self.completed_cycles,
            last_error: self.last_error.clone(),
        }
    }

    pub fn start(&mut self, mode: AppMode, policy: StopPolicy) -> Result<RuntimePhase, RuntimeError> {
        if self.phase != RuntimePhase::Idle {
            return Err(RuntimeError::Busy(self.phase));
        }
        policy.validate()?;

        self.last_error = None;
        self.completed_cycles = 0;
        self.selected_mode = mode;

        self.phase = match mode {
            AppMode::Mouse => RuntimePhase::RunningMouse,
            AppMode::Keyboard => RuntimePhase::RunningKeyboard,
            AppMode::Macro => RuntimePhase::PlayingMacro,
        };

        Ok(self.phase)
    }

    pub fn start_recording(&mut self, mode: AppMode) -> Result<RuntimePhase, RuntimeError> {
        if self.phase != RuntimePhase::Idle {
            return Err(RuntimeError::Busy(self.phase));
        }

        if mode == AppMode::Macro {
            self.phase = RuntimePhase::Recording;
            self.selected_mode = AppMode::Macro;
            Ok(self.phase)
        } else {
            Err(RuntimeError::InvalidTransition)
        }
    }

    pub fn finish_recording(&mut self) -> Result<RuntimePhase, RuntimeError> {
        if self.phase != RuntimePhase::Recording {
            return Err(RuntimeError::InvalidTransition);
        }
        self.phase = RuntimePhase::Idle;
        Ok(self.phase)
    }

    pub fn begin_stop(&mut self) -> Result<RuntimePhase, RuntimeError> {
        match self.phase {
            RuntimePhase::RunningMouse | RuntimePhase::RunningKeyboard | RuntimePhase::PlayingMacro | RuntimePhase::Recording => {
                self.phase = RuntimePhase::Stopping;
                Ok(self.phase)
            }
            _ => Err(RuntimeError::InvalidTransition),
        }
    }

    pub fn finish_stop(&mut self) -> Result<RuntimePhase, RuntimeError> {
        if self.phase != RuntimePhase::Stopping {
            return Err(RuntimeError::InvalidTransition);
        }
        self.phase = RuntimePhase::Idle;
        Ok(self.phase)
    }

    pub fn fail(&mut self, message: String) -> Result<RuntimePhase, RuntimeError> {
        self.last_error = Some(message);
        self.phase = RuntimePhase::Error;
        Ok(self.phase)
    }

    pub fn recover_idle(&mut self) -> Result<RuntimePhase, RuntimeError> {
        if self.phase != RuntimePhase::Error {
            return Err(RuntimeError::InvalidTransition);
        }
        self.phase = RuntimePhase::Idle;
        Ok(self.phase)
    }

    pub fn increment_cycle(&mut self) {
        self.completed_cycles += 1;
    }
}

#[derive(Debug)]
pub struct RuntimeCoordinator {
    model: Mutex<RuntimeModel>,
    generation: AtomicU64,
}

impl Default for RuntimeCoordinator {
    fn default() -> Self {
        Self::new()
    }
}

impl RuntimeCoordinator {
    pub fn new() -> Self {
        Self {
            model: Mutex::new(RuntimeModel::default()),
            generation: AtomicU64::new(0),
        }
    }

    pub async fn snapshot(&self) -> RuntimeSnapshot {
        self.model.lock().await.snapshot()
    }

    pub async fn start(&self, mode: AppMode, policy: StopPolicy) -> Result<(RuntimePhase, u64), RuntimeError> {
        let mut model = self.model.lock().await;
        let result = model.start(mode, policy);

        match result {
            Ok(phase) => {
                let gen = self.generation.fetch_add(1, Ordering::SeqCst) + 1;
                Ok((phase, gen))
            }
            Err(e) => Err(e),
        }
    }

    pub async fn start_recording(&self, mode: AppMode) -> Result<(RuntimePhase, u64), RuntimeError> {
        let mut model = self.model.lock().await;
        let result = model.start_recording(mode);

        match result {
            Ok(phase) => {
                let gen = self.generation.fetch_add(1, Ordering::SeqCst) + 1;
                Ok((phase, gen))
            }
            Err(e) => Err(e),
        }
    }

    pub async fn finish_recording(&self) -> Result<RuntimePhase, RuntimeError> {
        self.model.lock().await.finish_recording()
    }

    pub async fn begin_stop(&self) -> Result<RuntimePhase, RuntimeError> {
        self.model.lock().await.begin_stop()
    }

    pub async fn finish_stop(&self) -> Result<RuntimePhase, RuntimeError> {
        self.model.lock().await.finish_stop()
    }

    pub async fn fail(&self, message: String) -> Result<RuntimePhase, RuntimeError> {
        self.model.lock().await.fail(message)
    }

    pub async fn recover_idle(&self) -> Result<RuntimePhase, RuntimeError> {
        self.model.lock().await.recover_idle()
    }

    pub async fn increment_cycle(&self) {
        self.model.lock().await.increment_cycle();
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn default_is_idle_with_mouse_and_zero_cycles() {
        let runtime = RuntimeModel::default();
        assert_eq!(runtime.phase, RuntimePhase::Idle);
        assert_eq!(runtime.selected_mode, AppMode::Mouse);
        assert_eq!(runtime.completed_cycles, 0);
        assert_eq!(runtime.last_error, None);
    }

    #[test]
    fn validates_stop_policy_rejects_zero_count() {
        assert_eq!(StopPolicy::RepeatCount(0).validate(), Err(RuntimeError::InvalidCount));
    }

    #[test]
    fn validates_stop_policy_rejects_zero_duration() {
        assert_eq!(StopPolicy::DurationMs(0).validate(), Err(RuntimeError::InvalidDuration));
    }

    #[test]
    fn validates_stop_policy_accepts_positive_count() {
        assert!(StopPolicy::RepeatCount(1).validate().is_ok());
    }

    #[test]
    fn validates_stop_policy_accepts_positive_duration() {
        assert!(StopPolicy::DurationMs(1).validate().is_ok());
    }

    #[test]
    fn validates_stop_policy_accepts_until_stopped() {
        assert!(StopPolicy::UntilStopped.validate().is_ok());
    }

    #[test]
    fn start_from_idle_clears_error_and_resets_cycles() {
        let mut runtime = RuntimeModel::default();
        runtime.last_error = Some("previous error".to_string());
        runtime.completed_cycles = 5;

        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();

        assert_eq!(runtime.last_error, None);
        assert_eq!(runtime.completed_cycles, 0);
        assert_eq!(runtime.phase, RuntimePhase::RunningMouse);
    }

    #[test]
    fn rejects_reentrant_start() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();
        assert_eq!(
            runtime.start(AppMode::Keyboard, StopPolicy::RepeatCount(2)),
            Err(RuntimeError::Busy(RuntimePhase::RunningMouse))
        );
    }

    #[test]
    fn start_with_mouse_selects_running_mouse_phase() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();
        assert_eq!(runtime.phase, RuntimePhase::RunningMouse);
    }

    #[test]
    fn start_with_keyboard_selects_running_keyboard_phase() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Keyboard, StopPolicy::UntilStopped).unwrap();
        assert_eq!(runtime.phase, RuntimePhase::RunningKeyboard);
    }

    #[test]
    fn start_with_macro_selects_playing_macro_phase() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Macro, StopPolicy::UntilStopped).unwrap();
        assert_eq!(runtime.phase, RuntimePhase::PlayingMacro);
    }

    #[test]
    fn recording_starts_only_from_idle_for_macro_mode() {
        let mut runtime = RuntimeModel::default();

        assert!(runtime.start_recording(AppMode::Macro).is_ok());
        assert_eq!(runtime.phase, RuntimePhase::Recording);
    }

    #[test]
    fn recording_rejected_for_non_macro_mode() {
        let mut runtime = RuntimeModel::default();

        assert_eq!(
            runtime.start_recording(AppMode::Mouse),
            Err(RuntimeError::InvalidTransition)
        );
    }

    #[test]
    fn recording_rejected_when_already_running() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();

        assert_eq!(
            runtime.start_recording(AppMode::Macro),
            Err(RuntimeError::Busy(RuntimePhase::RunningMouse))
        );
    }

    #[test]
    fn running_and_recording_are_mutually_exclusive() {
        let mut runtime = RuntimeModel::default();

        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();
        assert_eq!(
            runtime.start_recording(AppMode::Macro),
            Err(RuntimeError::Busy(RuntimePhase::RunningMouse))
        );

        runtime.begin_stop().unwrap();
        runtime.finish_stop().unwrap();

        runtime.start_recording(AppMode::Macro).unwrap();
        assert_eq!(
            runtime.start(AppMode::Mouse, StopPolicy::UntilStopped),
            Err(RuntimeError::Busy(RuntimePhase::Recording))
        );
    }

    #[test]
    fn begin_stop_from_running() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();

        assert_eq!(runtime.begin_stop(), Ok(RuntimePhase::Stopping));
    }

    #[test]
    fn begin_stop_from_recording() {
        let mut runtime = RuntimeModel::default();
        runtime.start_recording(AppMode::Macro).unwrap();

        assert_eq!(runtime.begin_stop(), Ok(RuntimePhase::Stopping));
    }

    #[test]
    fn begin_stop_rejected_from_idle() {
        let mut runtime = RuntimeModel::default();
        assert_eq!(runtime.begin_stop(), Err(RuntimeError::InvalidTransition));
    }

    #[test]
    fn finish_stop_changes_stopping_to_idle() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();
        runtime.begin_stop().unwrap();

        assert_eq!(runtime.finish_stop(), Ok(RuntimePhase::Idle));
        assert_eq!(runtime.phase, RuntimePhase::Idle);
    }

    #[test]
    fn finish_stop_rejected_from_idle() {
        let mut runtime = RuntimeModel::default();
        assert_eq!(runtime.finish_stop(), Err(RuntimeError::InvalidTransition));
    }

    #[test]
    fn fail_records_error_and_enters_error_phase() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();

        assert_eq!(runtime.fail("execution failed".to_string()), Ok(RuntimePhase::Error));
        assert_eq!(runtime.last_error, Some("execution failed".to_string()));
        assert_eq!(runtime.phase, RuntimePhase::Error);
    }

    #[test]
    fn recover_idle_from_error_clears_phase_but_keeps_error() {
        let mut runtime = RuntimeModel::default();
        runtime.fail("some error".to_string()).unwrap();

        assert_eq!(runtime.recover_idle(), Ok(RuntimePhase::Idle));
        assert_eq!(runtime.phase, RuntimePhase::Idle);
        assert_eq!(runtime.last_error, Some("some error".to_string()));
    }

    #[test]
    fn recover_idle_rejected_from_idle() {
        let mut runtime = RuntimeModel::default();
        assert_eq!(runtime.recover_idle(), Err(RuntimeError::InvalidTransition));
    }

    #[test]
    fn snapshot_reflects_phase_mode_cycles_and_error() {
        let mut runtime = RuntimeModel::default();
        runtime.selected_mode = AppMode::Keyboard;
        runtime.completed_cycles = 42;
        runtime.last_error = Some("test error".to_string());

        let snapshot = runtime.snapshot();

        assert_eq!(snapshot.phase, RuntimePhase::Idle);
        assert_eq!(snapshot.selected_mode, AppMode::Keyboard);
        assert_eq!(snapshot.completed_cycles, 42);
        assert_eq!(snapshot.last_error, Some("test error".to_string()));
    }

    #[test]
    fn mode_change_does_not_start_execution() {
        let mut runtime = RuntimeModel::default();
        runtime.selected_mode = AppMode::Keyboard;

        assert_eq!(runtime.phase, RuntimePhase::Idle);
        assert_eq!(runtime.selected_mode, AppMode::Keyboard);
    }

    #[test]
    fn invalid_transition_does_not_mutate_state() {
        let mut runtime = RuntimeModel::default();
        runtime.start(AppMode::Mouse, StopPolicy::UntilStopped).unwrap();
        let original_phase = runtime.phase;

        let result = runtime.start(AppMode::Keyboard, StopPolicy::RepeatCount(2));

        assert!(result.is_err());
        assert_eq!(runtime.phase, original_phase);
    }

    #[test]
    fn increment_cycle_increments_completed_cycles() {
        let mut runtime = RuntimeModel::default();
        runtime.increment_cycle();
        runtime.increment_cycle();
        assert_eq!(runtime.completed_cycles, 2);
    }
}
